...
public final class FuxUiAnimation {
    ...
    /** Frame-rate independent-ish exponential smoothing for HUD/world motion. */
    public static float smooth(float current, float target, float speed, float deltaSeconds) {
        float factor = 1.0F - (float) Math.exp(-Math.max(0.0F, speed) * Math.max(0.0F, deltaSeconds));
